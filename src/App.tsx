...
import { CoinWorkspace } from './CoinWorkspace';
...
function App() {
  ...
  const scheduler = useScheduler(state.wheels);
  const history = useMemo(() => [...state.history, ...scheduler.snapshot.results.filter((item) => Date.parse(item.completedAt) <= Date.now()).map((item): DecisionHistoryItem => ({
    id: item.id, type: 'scheduled', result: item.result, createdAt: item.completedAt, sourceName: item.wheel.name
  }))].sort((a, b) => Date.parse(b.createdAt) - Date.parse(a.createdAt)), [state.history, scheduler.snapshot.results]);

  useEffect(() => { setSoundEnabled(state.soundEnabled); }, [state.soundEnabled]);
  useEffect(() => () => {
    stopSpinTracking();
    if (numberTimerRef.current !== null) clearInterval(numberTimerRef.current);
  }, []);
  ...
  function spinWheel() {
    ...
    // APK resets its controller before each draw and uses 5 * (floor(seconds / 5) + 1) turns.
    const turns = 5 * (Math.floor(activeWheel.spinDurationMs / 5000) + 1);
    const targetRotation = turns * 360 + targetWithinTurn;
    flushSync(() => {
      setIsWheelResetting(true);
      setRotation(0);
      setIsSpinning(true);
    ...
    // Commit the reset even when the next random draw has the same landing angle.
    if (wheelRef.current) void wheelRef.current.getBoundingClientRect();
    ...
    const updateRollingResult = () => {
      ...
      const id = getPointedSegment(wheelRef.current, segments)?.option.id;
      if (id && id !== lastPointedId) { lastPointedId = id; playSound('tick'); }
      if (pointed) {
  ...
  function generateNumbers() {
    ...
    const config = { ...state.numberConfig };
    const intervalMs = 100;
    const rollMs = config.animationSeconds * 1000;
    let elapsed = 0;
    let nextUpdate = 0;
    ...
    const timer = window.setInterval(() => {
      ...
      if (elapsed >= rollMs) {
        ...
        const finalResult = createRandomNumbers(config);
        const shown = config.showOrdered ? [...finalResult].sort((a, b) => a - b) : finalResult;
        setNumberResult(shown);
        setIsNumberRolling(false);
        playSound('win');
        addHistory({ type: "number", result: shown, sourceName: `${config.min} ~ ${config.max}` });
      } else if (elapsed > nextUpdate) {
        const preview = createRandomNumbers(config);
        setNumberResult(config.showOrdered ? preview.sort((a, b) => a - b) : preview);
        playSound('tick');
        // APK CurvedTimer, VA 0x8116f8-0x811710: next = t + (t / T)^2 * T.
        nextUpdate = elapsed + elapsed * elapsed / rollMs;
      }
    }, intervalMs);
    numberTimerRef.current = timer;
  }

  async function clearHistory() {
    setHistoryError('');
    try {
      if (window.desktop) await window.desktop.clearScheduledHistory();
      setState((current) => ({ ...current, history: [] }));
    } catch (reason) { setHistoryError(`清空失败：${String(reason)}`); }
  }
  ...
  return (
    <main className="desktop-shell">
      <aside className="app-sidebar">
        ...
        />
        <label className="desktop-check sound-setting">{state.soundEnabled ? <Volume2 size={20} /> : <VolumeX size={20} />}音效
          <input type="checkbox" checked={state.soundEnabled} onChange={(event) => setState((current) => ({ ...current, soundEnabled: event.target.checked }))} />
        </label>
      </aside>

      <section className="workspace">
        <div hidden={section !== 'wheel'}>
          <WheelWorkspace
            wheel={activeWheel}
      ...
      <section className="wheel-stage-panel">
        <div className="workspace-toolbar">
          ...
          <div className="toolbar-actions">
            <button className="ghost-button" disabled={props.isSpinning} onClick={() => setShowOptions(true)}><List size={20} />选项</button>
            <button className="ghost-button" disabled={props.isSpinning} onClick={props.onEdit}>
              <Edit3 size={20} />
              编辑
            </button>
          </div>
        ...
        <div className="desktop-wheel-actions">
          ...
          <button className="round-button" aria-label="编辑" disabled={props.isSpinning} onClick={props.onEdit}>
            <Edit3 size={24} />
          </button>
        </div>
      </section>

      {showOptions && <Modal title="转盘选项" onClose={() => setShowOptions(false)}>
        <div className="wheel-option-float">{props.wheel.options.map((option) => <label key={option.id} className="floating-option">
          <input type="checkbox" checked={!props.disabledOptionIds.includes(option.id)} disabled={props.isSpinning} onChange={() => props.onToggleOption(option.id)} />
          <i style={{ background: option.color }} /><span>{option.label}</span><small>{formatPercent(option.weight, props.wheel.options.reduce((sum, item) => sum + item.weight, 0))}</small>
        </label>)}</div>
        <WheelSummary wheel={props.wheel} history={props.recentHistory} onEdit={() => { setShowOptions(false); props.onEdit(); }} />
      </Modal>}
      {props.editingWheelId && <WheelEditorDialog key={props.editingWheelId} wheel={props.editingWheel} canDelete={props.canDelete} onSave={props.onSaveWheel} onClose={props.onCloseEditor} onDelete={props.onDeleteWheel} />}
    </div>
  );
}

export function DecisionWheel(props: {
  segments: WheelSegment[];
  rotation: number;
  ...
  return (
    <div className="decision-wheel-button">
      <div className="decision-wheel-shadow">
        ...
        <button className="decision-hub-button" disabled={props.segments.every((segment) => props.disabledOptionIds.includes(segment.option.id))} onClick={props.onSpin} aria-label="旋转转盘">
...
function WheelEditorDialog(props: { wheel: WheelConfig; canDelete: boolean; onSave: (wheel: WheelConfig) => void; onClose: () => void; onDelete: () => void }) {
  const [draft, setDraft] = useState(() => structuredClone(props.wheel));
  const [error, setError] = useState('');
  const [confirmDelete, setConfirmDelete] = useState(false);
  return <Modal title="编辑转盘" onClose={props.onClose}>
    {error && <p className="number-error" role="alert">{error}</p>}
    <WheelEditor wheel={draft} canDelete={props.canDelete} onChange={setDraft} onCancel={props.onClose} onDelete={() => setConfirmDelete(true)} onSave={(wheel) => {
      if (!wheel.name.trim() || wheel.options.some((option) => !option.label.trim())) { setError('名称和选项不能为空'); return; }
      props.onSave(wheel);
    }} />
    {confirmDelete && <Modal title="删除转盘" onClose={() => setConfirmDelete(false)}>
      <p>确定删除「{props.wheel.name}」？关联的定时任务也会删除。</p>
      <div className="editor-actions">
        <button className="ghost-button" onClick={() => setConfirmDelete(false)}>取消</button>
        <button className="danger-button" onClick={props.onDelete}><Trash2 size={18} />确认删除</button>
      </div>
    </Modal>}
  </Modal>;
}
...
function WheelEditor(props: {
  ...
  return (
    <div className="editor-panel">
      ...
      {colorEditorOption && (
        <Modal title="颜色选择" onClose={() => setColorEditorOptionId(null)}>
      ...
      {weightEditorOption && (
        <Modal title="权重编辑" onClose={() => setWeightEditorOptionId(null)}>
...
function NumberWorkspace(props: {
  ...
  const visibleResult = props.result.length ? props.result : [42];
  ...
  return (
    <div className="number-workspace">
      <section className="number-main-panel">
        ...
        <button className="desktop-number-display" aria-label="生成随机数字" disabled={Boolean(props.error) || props.isRolling} onClick={props.onGenerate}>
          <span className={`number-results ${visibleResult.length === 1 ? 'single' : ''}`} aria-live={props.isRolling ? 'off' : 'polite'}>
            {visibleResult.map((number, index) => <span key={index} className="number-tile" style={visibleResult.length > 1 && String(number).length > 5 ? { fontSize: 18 } : undefined}>{number}</span>)}
          </span>
        </button>
      ...
      <aside className="number-settings-drawer">
        <div className="summary-panel">
          ...
          <fieldset className="number-config-form" disabled={props.isRolling}>
            <label className="desktop-field">
              最小值
  ...
  return (
    <section className="history-workspace">
      ...
      <div className="history-table">
        ...
        {props.history.map((item) => (
          <article key={item.id}>
            <span>{{ wheel: '转盘', number: '随机数字', coin: '抛硬币', scheduled: '定时转盘' }[item.type]}</span>
            <strong>{Array.isArray(item.result) ? item.result.join(", ") : item.result}</strong>
...
export function buildSegments(options: WheelOption[]): WheelSegment[] {
...
function getNumberError(config: NumberConfig): string {
  if (!Number.isInteger(config.min) || !Number.isInteger(config.max) || config.min < -999999 || config.max > 999999) {
    return "范围必须为 -999999 至 999999 之间的整数。";
  }
  if (config.min > config.max) {
    return "最小值不能大于最大值。";
  }
  if (!Number.isInteger(config.count) || config.count < 1 || config.count > 1000) {
    return "数量必须为 1 至 1000 之间的整数。";
  }
...
function createRandomNumbers(config: NumberConfig): number[] {
  ...
  // Partial Fisher-Yates keeps rolling large ranges proportional to the requested count.
  const swaps = new Map<number, number>();
  const output: number[] = [];
  const available = max - min + 1;
  for (let index = 0; index < count; index += 1) {
    const pick = randomInt(index, available - 1);
    output.push(min + (swaps.get(pick) ?? pick));
